...
void enemySet(float a, float r, float g, float b, float pibo, float t, float enemySpeed)
{
	...
	//게임 오버 구현
	gameOverCheck(-0.1*enemyT1 + enemy1Speed, "Enemy 1");
	gameOverCheck(-0.1*enemyT2 + enemy2Speed, "Enemy 2");
	gameOverCheck(-0.1*enemyT3 + enemy3Speed, "Enemy 3");
	gameOverCheck(-0.1*enemyT4 + enemy4Speed, "Enemy 4");

	//Enemy
	glPushMatrix();
	glTranslatef(a, (sin(t*00.1) + 1) / 6, -0.1*t + enemySpeed);
	// 1. 몸통
	glMaterialfv(GL_FRONT, GL_DIFFUSE, mat_diffuse1); // 추가
	glPushMatrix();
	glRotatef(-90, 1, 0, 0);
	...
	glutSolidSphere(0.08, 20, 20);
	glRotatef(sin(-t*0.1) * 45 - 45, 0.5, 0, 0);
...
void userSet(float a, float r, float g, float b)
{
	...
	//게임 오버 체크!!
	gameOverCheck(-0.1 + userMove, "User");
